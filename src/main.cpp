  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent (event)) {
      ...
      if (event.type == sf::Event::Resized)
        window.setSize (sf::Vector2u (static_cast<unsigned int> (X_PX / SCALE), 
                                      static_cast<unsigned int> (Y_PX / SCALE)));
    ...
    float t_step = 1.0 / (bezier.get_control_points().size() - 1);
    draw_control_lines (foreground, bezier, 0, t_step);

    // Draw points
    for (float t = 0; t <= time; t += STEP)
      draw_point (foreground, VEC_TO_CANVAS (bezier.get_point_at_T (t)), R_PX, c2);
    draw_point (foreground, VEC_TO_CANVAS (bezier.get_point_at_T (time)), R_PX * 2, c2);
